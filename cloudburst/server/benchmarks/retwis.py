...
def run(cloudburst_client, num_requests, sckt):
    ...
    # Hydro redis shim.
    # ----
    class CloudburstRedisShim:
        def __init__(self, cloudburst_user_library):
            self._cloudburst = cloudburst_user_library

        # Checking for existence of arbitrary keys.
        def exists(self, key):
            value_or_none = self._cloudburst.get(key)
            return value_or_none is not None

        ## Single value storage.
        # Retrieving arbitrary values that were stored by set().
        def get(self, key):
            value = self._cloudburst.get(key)
            if value is None: return None
            #value = pickle.loads(value)
            return value
        # def get_with_vc(self, key):
        #     if self.exists(key):
        #         vc, values = self._cloudburst.get(key)
        #         value = values[0]
        #         value = pickle.loads(value)
        #         return vc, value
        #     else:
        ...
        def set(self, key, value):
           # value = pickle.dumps(value)
            self._cloudburst.put(key, value)
        ...
        def causal_set(self, key, value, dependencies):
            ...
            self._cloudburst.causal_put(key, vector_clock, dependencies, value, client_id)
        ...
        def smembers(self, key):
            values = self._cloudburst.get(key)
            if values is None: return set()
            return set(val for val in values)
        # Set membership.
    ...
    ## Defining functions.
    def ccc_redis_exists(cloudburst, key):
        redis = CloudburstRedisShim(cloudburst)
        return str(redis.exists(key))
    def ccc_redis_get(cloudburst, key):
        redis = CloudburstRedisShim(cloudburst)
        return str(redis.get(key))
    def ccc_redis_set(cloudburst, key, value):
        redis = CloudburstRedisShim(cloudburst)
        redis.set(key, value)
        return 'success'
    def ccc_redis_incr(cloudburst, key):
        redis = CloudburstRedisShim(cloudburst)
        return str(redis.incr(key))
    def ccc_redis_sadd(cloudburst, key, value):
        redis = CloudburstRedisShim(cloudburst)
        redis.sadd(key, value)
        return 'success'
    def ccc_redis_smembers(cloudburst, key):
        redis = CloudburstRedisShim(cloudburst)
        return str(redis.smembers(key))
    def ccc_redis_lpush(cloudburst, key, value):
        redis = CloudburstRedisShim(cloudburst)
        redis.lpush(key, value)
        return 'success'
    def ccc_redis_lrange(cloudburst, key, begin, end):
        redis = CloudburstRedisShim(cloudburst)
        return str(redis.lrange(key, begin, end))
    def ccc_redis_llen(cloudburst, key):
        redis = CloudburstRedisShim(cloudburst)
        return str(redis.llen(key))
    # def ccc_global_timeline(cloudburst, page):
    #     redis = CloudburstRedisShim(cloudburst)
    #     return Timeline.page(redis, page)

    def ccc_user_create(cloudburst, user):
        redis = CloudburstRedisShim(cloudburst)
        # cloudburst.put('ccc_user', SetLattice({b'hi',}))
        # cloudburst.put('ccc_userb', OrderedSetLattice(ListBasedOrderedSet([b'hi'])))
        User.create(redis, user, 'password')
        return 'success'
    def ccc_user_timeline(cloudburst, user, page):
        redis = CloudburstRedisShim(cloudburst)
        #return pickle.dumps(User.timeline(redis, user, page))
        return User.timeline(redis, user, page)
    def ccc_user_timeline_anomalies(cloudburst, user):
        redis = CloudburstRedisShim(cloudburst)
        #return pickle.dumps(User.timeline_anomalies(redis, user))
        return User.timeline_anomalies(redis, user)
    def ccc_user_profile(cloudburst, user, page):
        pass
    def ccc_user_follow(cloudburst, user, target):
        redis = CloudburstRedisShim(cloudburst)
        User.follow(redis, user, target)
        return 'success'
    def ccc_post_create(cloudburst, user, post):
        redis = CloudburstRedisShim(cloudburst)
        Post.create(redis, user, post)
        return 'success'
    def ccc_reply_create(cloudburst, user, post, parent_cid):
        redis = CloudburstRedisShim(cloudburst)
        Post.create(redis, user, post, parent_cid)
        return 'success'
    def ccc_reply_create_causal(cloudburst, user, post, parent_cid):
        redis = CloudburstRedisShim(cloudburst)
        Post.create_causal(redis, user, post, parent_cid)
        return 'success'
    # def ccc_test_empty(cloudburst):
    #     return []
    fns = {
    ...
    if sckt is None:
        cfns = {
            fname: cloudburst_client.register(f, fname)
            for fname, f
            in fns.items()
        }
        ...
        cfns = {
            fname: cloudburst_client.get_function(fname)
            for fname, f
            in fns.items()
        }
        for fname, cf in cfns.items():
    ...
    if sckt is None:
        ...
        for i in range(num_pretweets):
            ...
            post = "{} says: I love cloudburst!".format(username)
